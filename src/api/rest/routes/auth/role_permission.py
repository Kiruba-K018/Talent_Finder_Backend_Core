...
role_permission_router = APIRouter(prefix="/api/v1/role-permission", tags=["Role Permission"])


@role_permission_router.get("/roles/", status_code=200, response_model=list[RoleResponse])
...
@role_permission_router.get("/roles/{role_id}", status_code=200, response_model=RoleResponse)
async def get_role(
    role_id: int,
    current_user=Depends(requires_admin),
    db: AsyncSession = Depends(get_db)
...
@role_permission_router.get("/permissions/", status_code=200, response_model=list[PermissionResponse])
...
@role_permission_router.get("/permissions/{permission_id}", status_code=200, response_model=PermissionResponse)
...
@role_permission_router.post("/permissions/", status_code=201, response_model=PermissionResponse)
...
@role_permission_router.post("/roles/{role_id}/permissions/{permission_id}", status_code=200)
async def assign_permission_to_role(
    ...
    try:
        mapping = await role_permission_service.assign_permission(db, role_id, permission_id)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
...
@role_permission_router.delete("/roles/{role_id}/permissions/{permission_id}", status_code=204)
async def remove_permission_from_role(
    ...
    success = await role_permission_service.remove_permission(db, role_id, permission_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
@role_permission_router.get("/roles/{role_id}/permissions/", status_code=200, response_model=list[PermissionResponse])
