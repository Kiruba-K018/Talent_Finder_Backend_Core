...
async def create_sourcing_config(
    ...
    Only admin users can create sourcing configurations. Configuration
    ...
    try:
        ...
        new_config = await create_sourcing_config_service(
            db, current_user.user_id, config_dict
        )
        return new_config
...
async def get_sourcing_config_by_id(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid config_id format") from err
    except Exception as err:
        raise HTTPException(status_code=400, detail=str(err)) from err


@sourcing_config_router.delete(
    "/", status_code=status.HTTP_200_OK, response_model=SourcingConfigDeleteResponse
)
async def deactivate_sourcing_config(
    ...
    Only admin users can deactivate configurations. Preserves configuration
    record in database.
    ...
    try:
        ...
        return SourcingConfigDeleteResponse(
            message="Sourcing configuration deactivated successfully"
        )
    except Exception as err:
        raise HTTPException(status_code=400, detail=str(err)) from err
...
async def update_sourcing_config(
    ...
    try:
        config_dict = config_data.model_dump()
        updated_config = await create_sourcing_config_service(
            db, current_user.user_id, config_dict
        )
