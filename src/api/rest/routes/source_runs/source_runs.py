...
from src.api.rest.dependencies import _db
from src.core.services.source_run.source_run_services import (
    create_source_run_record_service,
    delete_source_run_service,
    fetch_all_source_runs_service,
    fetch_one_source_run_service,
)
...
@source_run_router.post(
    "/", status_code=status.HTTP_201_CREATED, response_model=SourceRunResponse
)
async def create_source_run_record(
    config: dict, db: AsyncSession = _db
) -> SourceRunResponse:
    ...
    try:
        ...
        if isinstance(result, dict) and "source_run_id" in result:
            return SourceRunResponse(
                source_run_id=result["source_run_id"], config=config
            )
        return result
...
@source_run_router.get(
    "/", status_code=status.HTTP_200_OK, response_model=SourceRunListResponse
)
async def get_all_source_runs(
    db: AsyncSession = _db,
) -> SourceRunListResponse:
...
@source_run_router.get(
    ...
    response_model=SourceRunDetailResponse,
)
async def get_one_source_run(
    source_run_id: uuid.UUID, db: AsyncSession = _db
) -> SourceRunDetailResponse:
...
@source_run_router.delete(
    "/{source_run_id}",
    status_code=status.HTTP_200_OK,
    response_model=SourceRunDeleteResponse,
)
async def delete_source_run(
    ...
    try:
        ...
        return SourceRunDeleteResponse(
            message=f"Source run {source_run_id} deleted successfully"
        )
    except Exception as err:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(err)
