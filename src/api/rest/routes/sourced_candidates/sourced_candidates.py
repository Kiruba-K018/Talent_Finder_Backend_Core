...
from src.api.rest.dependencies import _mongodb
...
@sourced_candidates_router.get(
    "/", status_code=status.HTTP_200_OK, response_model=SourcedCandidateListResponse
)
async def get_all_sourced_candidates(
    db=_mongodb,
) -> SourcedCandidateListResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Error retrieving candidates"
        ) from err
...
@sourced_candidates_router.get(
    ...
    response_model=SourcedCandidateDetailResponse,
)
async def get_sourced_candidate(
    candidate_id: str, db=_mongodb
) -> SourcedCandidateDetailResponse:
...
@sourced_candidates_router.get(
    "/by-source-run/{source_run_id}",
    status_code=status.HTTP_200_OK,
    response_model=SourcedCandidateListResponse,
)
async def get_sourced_candidates_by_source_run(
    source_run_id: str, db=_mongodb
) -> SourcedCandidateListResponse:
...
@sourced_candidates_router.post(
    "/",
    status_code=status.HTTP_201_CREATED,
    response_model=CreateSourcedCandidateResponse,
)
async def create_sourced_candidate(
    sourced_candidate: dict, db=_mongodb
) -> CreateSourcedCandidateResponse:
...
@sourced_candidates_router.delete(
    "/{candidate_id}/",
    status_code=status.HTTP_200_OK,
    response_model=CreateSourcedCandidateResponse,
)
async def delete_sourced_candidate(
    candidate_id: str, db=_mongodb
) -> CreateSourcedCandidateResponse:
    ...
    return CreateSourcedCandidateResponse(
