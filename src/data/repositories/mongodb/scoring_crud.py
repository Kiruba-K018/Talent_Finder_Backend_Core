...
async def save_candidate_scores_with_fresh_client(scores: list[dict]) -> bool:
    ...
    try:
        ...
        if scores:
            logger.info(
                f"Saving {len(scores)} candidate scores to MongoDB with fresh client"
            )
            result = await collection.insert_many(scores)
            logger.info(
                f"Successfully saved {len(result.inserted_ids)} scores with "
                f"fresh client"
            )
        return True
    except Exception as e:
        logger.error(
            f"Failed to save candidate scores with fresh client: {e}", exc_info=True
        )
        return False
    finally:
        if fresh_client:
...
async def get_candidate_score(candidate_id: str, job_id: str) -> dict | None:
    try:
        ...
        logger.error(
            f'Error retrieving candidate score for candidate {candidate_id} "'
            f"and job {job_id}: {e}"
        )
        return None
