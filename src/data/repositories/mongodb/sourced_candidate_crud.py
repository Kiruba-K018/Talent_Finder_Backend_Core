...
async def get_sourced_candidates_with_fresh_client(job_id: uuid.UUID) -> list:
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched {len(candidates)} candidates for job {job_id}"
        )
        return candidates
...
async def get_candidate_data_with_fresh_client(candidate_id: str) -> dict | None:
    ...
    try:
        ...
        collection = db["sourced_candidates"]
        logger.debug(
            f"Querying sourced_candidates for candidate_id: {candidate_id} "
        ...
        if fresh_client:
            fresh_client.close()
            logger.debug(
                f"Closed fresh MongoDB connection for candidate {candidate_id}"
            )
...
async def get_sourced_candidates(job_id: uuid.UUID) -> list:
    try:
        ...
        logger.debug(
            f"Querying sourced_candidates collection for job_id: {job_id_str} "
            f"(type: {type(job_id_str)})"
        )
        candidates = await collection.find({}).to_list(length=None)
        return candidates
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if result.inserted_id:
                logger.debug(
                    f"Inserted sourced candidate with ID: {result.inserted_id}"
                )
            ...
            if "duplicate key" in str(insert_error).lower():
                logger.debug(
                    f"Candidate already exists, attempting update instead: "
                ...
                logger.debug(
                    f"Updated/upserted candidate: {candidate_data.get('candidate_id')}"
                )
                return candidate_data
            else:
                raise
...
async def delete_sourced_candidate(candidate_id: str) -> bool:
    try:
        ...
        if result.deleted_count == 1:
            ...
            logger.warning(
                f"No sourced candidate found to delete with ID: {candidate_id}"
            )
            return False
    except Exception as e:
        logger.error(
            f"Error deleting sourced candidate with ID {candidate_id}: {e}",
            exc_info=True,
        )
        return False
...
async def get_sourced_candidates_by_source_run(source_run_id: str) -> list:
    try:
        ...
        collection = db["sourced_candidates"]
        logger.debug(
            f"Querying sourced_candidates for source_run_id: {source_run_id} "
            f"(type: {type(source_run_id).__name__})"
        )
        candidates = await collection.find({"source_run_id": source_run_id}).to_list(
            length=None
        )
        logger.debug(
            f"Found {len(candidates)} candidates for source_run_id: {source_run_id}"
        )
        return candidates
    except Exception as e:
        logger.error(
            f"Error fetching sourced candidates for source run {source_run_id}: {e}",
            exc_info=True,
        )
        return []
