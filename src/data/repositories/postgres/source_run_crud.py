...
async def insert_source_run_record(db: AsyncSession, source_run_data: dict) -> bool:
    ...
    try:
        ...
        data_to_insert = {
            "source_run_id": uuid.UUID(source_run_data["source_run_id"]) if isinstance(source_run_data.get("source_run_id"), str) else source_run_data.get("source_run_id"),
            "platform_id": uuid.UUID(source_run_data["platform_id"]) if isinstance(source_run_data.get("platform_id"), str) else source_run_data.get("platform_id"),
            "config_id": uuid.UUID(source_run_data["config_id"]) if isinstance(source_run_data.get("config_id"), str) else source_run_data.get("config_id"),
            "status": source_run_data.get("status", "completed"),
            "number_of_resume_fetched": int(source_run_data.get("number_of_resume_fetched", 0)),
...
async def upsert_source_run_record(db: AsyncSession, source_run_data: dict) -> bool:
    ...
    try:
        source_run_id = uuid.UUID(source_run_data.get("source_run_id")) if isinstance(source_run_data.get("source_run_id"), str) else source_run_data.get("source_run_id")
        ...
        if existing_run:
            ...
            existing_run.number_of_resume_fetched = int(source_run_data.get("number_of_resume_fetched", existing_run.number_of_resume_fetched))
            existing_run.completed_at = source_run_data.get("completed_at") or datetime.utcnow()
    ...
    try:
        ...
        if source_run:
            return {
                ...
                "platform_id": str(source_run.platform_id) if source_run.platform_id else None,
                ...
                "config_id": str(source_run.config_id) if source_run.config_id else None,
