...
class SourcingConfigCreate(BaseModel):
    frequency: str = Field(
        ..., description="Frequency of sourcing runs (e.g., 'daily', 'weekly')"
    )
    scheduled_time: str | time = Field(
        ..., description="Time of day to run the sourcing (e.g., '02:00:00')"
    )
    scheduled_day: str | None = Field(
        None, description="Day of the week for weekly runs (e.g., 'Monday')"
    )
    ...
    def validate_scheduled_time(cls, v):
        ...
        raise ValueError(
            f"scheduled_time must be str or time object, got {type(v).__name__}"
        )
